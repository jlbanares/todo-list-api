...
from todo_list.common.exceptions import ApplicationError
...
def exception_errors_format_handler(exc, context):
    ...
    if response is None:
        if isinstance(exc, ApplicationError):
            data = {"message": exc.message, "code": exc.code, "extra": exc.extra}
            # To be compatible with previous format of response
